...
#redistributes those blocks among the banks. To do this, it removes all of the blocks from the selected bank, 
...
#=============================================================================
def redistribute(memory_array):
#
# Find the highest number in the memory array and split it between the 
# banks evenly
#
#-----------------------------------------------------------------------------
    index, value = max(enumerate(memory_array), key=operator.itemgetter(1))
    memory_array[index] = 0

    size = len(memory_array)
    index += 1
    while (value >0):
        if index >= size:
            index = 0
        memory_array[index] += 1
        index += 1
        value -= 1
    return memory_array
...
#-----------------------------------------------------------------------------
    # My list of data
    memory_array = [11, 11, 13, 7, 0, 15, 5, 5, 4, 4, 1, 1, 7, 1, 15, 11]
    memory_array_list = []
    memory_array_list.append(copy.copy(memory_array))
    index = 0
    while (True):
        memory_array = redistribute(memory_array)
        if memory_array in memory_array_list:
            index = memory_array_list.index(memory_array)
            print ("DONE")
            break
        else :
            memory_array_list.append(copy.copy(memory_array))

    # The cycle length is the total length minus the index
    return [len(memory_array_list), (len(memory_array_list) - index)]

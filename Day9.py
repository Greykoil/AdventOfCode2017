...
#Your goal is to find the total score for all groups in your input. Each group is assigned a score which is one more than the score of the group that immediately contains it. 
...
from FileReader import raw_read_file

#=============================================================================
def run_day_9():
#
#-----------------------------------------------------------------------------
    data = raw_read_file("Day9_Data.txt")

    parent_nesting = 0
    scores = []
    garbage_count = []
    read_group(data, parent_nesting, scores, garbage_count)

    total = 0
    for score in garbage_count:
        total += score
    return total

#=============================================================================
def read_group(stream, parent_nesting, scores, garbage_count):
#
#-----------------------------------------------------------------------------
    while (len(stream) > 0):
        stream, char = next_char(stream)
        # We might be at a sub group, at a garbage string or at a close group
        if (char == "{"):
            stream = read_group(stream, parent_nesting + 1, scores, garbage_count)
        elif (char == "<"):
            stream = read_garbage(stream, garbage_count)
        elif (char == "}"):
            scores.append(parent_nesting)
            return stream


#=============================================================================
def read_garbage(stream, garbage_count):
#
#
#-----------------------------------------------------------------------------
    current = 0
...
#
# Read the next char. Move past !
#
#-----------------------------------------------------------------------------
    if (stream[0] == "!"):
        stream = stream[2:]
        return next_char(stream)
    else :
        char = stream[0]
        stream = stream[1:]
        return stream, char